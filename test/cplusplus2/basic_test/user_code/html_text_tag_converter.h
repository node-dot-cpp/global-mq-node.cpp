...
std::string htmltag_to_htmlstring(basic_test::HtmlTag_subscriber& htmltag)
{
    std::string output;
    basic_test::HtmlTextOrTags_subscriber_base::Variants var = htmltag.get_tags()->currentVariant();

    // add opened tag
    if (!htmltag.get_name().empty())
    {
        output += "<" + htmltag.get_name();

        if (!htmltag.get_properties().empty())
        {
            for (auto& prop : htmltag.get_properties())
            {
                output += prop->get_name() + "=" + prop->get_value();
            }
        }
        output += ">";
    }
    ...
    {
        for (auto& tag : htmltag.get_tags()->get_tags())
        {
            //add text inside tag
    ...
    {
        output += htmltag.get_tags()->get_str();
    }

    // add closed tag
    if (!htmltag.get_name().empty())
    {
        if (htmltag.get_name() != "P") // condition for exceptions of singletags
        {
            output += "</" + htmltag.get_name() + ">";
        }
    }
